var webgl_fluid_settings = {"color_type":"simple","color":"#C64291","resolution":"256","velocity":"2.9","density":"1.73","pressure":"0.18","splat_radius":"0.67","backspace_animation":"yes"};
  (function ($) {
    "use strict";
  ...
  `);
    ...
    function fluidAnimation(reinit = false)
    {
      if(reinit){
        ...
        jQuery( "#wgl-webgl-fluid_2" ).remove();
      ...
      initCanvas(document.getElementById('wgl-webgl-fluid_2'), {
        IMMEDIATE: false, // Whether to trigger multiple random splats when initialized
        TRIGGER: 'hover',
        SIM_RESOLUTION: reinit ? reDrawResolution : parseInt(webgl_fluid_settings.resolution),
        DYE_RESOLUTION: 512,
    ...
    document.addEventListener('mousemove', event => {
      if (event.isTrusted && newEvent) {
        document.getElementById('wgl-webgl-fluid_2').dispatchEvent(newEvent);
      }
    })
